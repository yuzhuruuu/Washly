...
<body class="bg-[#F8FAFC] h-screen font-sans text-slate-800 flex overflow-hidden w-full">
    ...
    <aside class="w-64 bg-white border-r border-gray-100 flex flex-col h-full shrink-0 relative z-20 overflow-hidden">
        ...
        <nav class="flex-1 px-4 space-y-2 w-full overflow-y-auto no-scrollbar">
            ...
            {{-- MENU RIWAYAT (Aktif) --}}
            <a href="{{ route('kurir.riwayat') }}" class="flex items-center gap-3 px-4 py-3 bg-blue-50 text-[#0074A6] rounded-xl font-bold text-sm transition shadow-sm border border-blue-100 w-full">
            ...
            <div class="mb-10 w-full max-w-sm">
                <div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex items-center gap-5 w-full relative overflow-hidden">
                    ...
                    <div class="flex-1 min-w-0 relative z-10">
                        ...
                        {{-- SINKRONISASI: Menghitung jumlah riwayat asli dari database --}}
                        <h3 class="text-2xl font-black text-gray-800 truncate">{{ $riwayat_tugas->count() }} Tugas</h3>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden w-full">
                <div class="overflow-x-auto w-full">
                    <table class="w-full text-left border-collapse min-w-[800px]">
                        ...
                        <tbody class="text-sm">
                            {{-- SINKRONISASI: Looping data riwayat tugas asli dari database lokal --}}
                            @forelse ($riwayat_tugas as $tugas)
                                    ...
                                    <td class="py-4 px-8">
                                        <div class="flex items-center gap-4">
                                            <div class="w-9 h-9 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-xs font-bold shrink-0">
                                                {{ strtoupper(substr($tugas->pelanggan?->nama ?? 'P', 0, 2)) }}
                                            </div>
                                            <span class="font-semibold text-gray-700 truncate">{{ $tugas->pelanggan?->nama ?? 'Pelanggan Washly' }}</span>
                                        </div>
                                    </td>
                                    <td class="py-4 px-6">
                                        <div class="flex items-center gap-2 text-gray-600 font-medium">
                                            @if(($tugas->status_pengiriman ?? '') == 'delivery' || $tugas->status == 'selesai')
                            ...
                            @empty
                                {{-- Jika belum ada riwayat tugas sama sekali --}}
                                <tr>
                                    <td colspan="4" class="py-8 text-center text-gray-400 font-medium">Belum ada riwayat tugas yang diselesaikan.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="px-8 py-10 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-6 bg-white w-full">
                    {{-- SINKRONISASI: Info jumlah total dinamis --}}
                    <p class="text-sm text-gray-500 font-medium">Menampilkan {{ $riwayat_tugas->count() }} dari {{ $riwayat_tugas->count() }} riwayat</p>
